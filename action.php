...
$senhaA = $row[0];
    ...
    $sql = "UPDATE user SET complemento='$complemento', cep='$cep' , numero='$numero', nome='$nome', cidade='$cidade',uf='$uf',rua='$rua', password='$senha'
    WHERE email='$user' ";
    $resltU = $conn->query($sql);
    $status =  true;
    //echo("Error description: " . $resltU -> error);
    echo "<script>location='dashboard.php?status=true'</script>";
  ...
  if (isset($_GET["masculino"])) {
   echo $sexo = $_GET['sexo'];
   echo $city = $_GET['city'];
    echo $estilo= $_GET['estilo'];
   echo $clima=$_GET['clima'];
   echo $ambiente=$_GET['ambiente'];
   echo $atividade=$_GET['atividade'];
...
if(isset($_POST["register"]))
{
    ...
    if($cpass==$pass)
    {
        if(!$rec)
        {
            $send="INSERT INTO user(email,nome,password,cpf,nascimento)VALUES('$email','$nome','$pass','$cpf','$nascimento')";
            $exec=db::query($send);
...
if(isset($_GET["fav2"])&&isset($_GET["st"]))
{
    ...
    if($st==1 )
    {
        ...
        echo "<script>location='index.php#fem'</script> . $p_id " ;
...
if(isset($_GET["fav"])&&isset($_GET["st"]))
{
    ...
    {
echo "foi";
       $sql = "SELECT p_id from user WHERE u_id='$u_id' ";
       $result= $conn->query($sql);
       $row = mysqli_fetch_row($result);
        ...
        $add="INSERT INTO fav(p_id,u_id)VALUES('$p_id','$u_id')";
        $exc=db::query($add);
       echo "<script>location='perfumes.php'</script>";
    }
    else if($st==0)
    {
        $add="DELETE FROM fav WHERE u_id='$u_id' and p_id='$p_id'";
